...
* @package reportes
*/
class DistComercialVendedor extends Reportes
{
	/**
	...
	*/
	...
	{
		$sql="SELECT 
m.name||' '||m.lastname AS vendedor, c.name AS operador, m.position, y.name as company, z.name as monetizable, tp.name as termino_pago, d.days as dias_disputa, cl.amount as limite_credito, pl.amount as limite_compra
...
UNION
select m.name||' '||m.lastname AS vendedor, c.name AS operador, m.position,'Sin Asignar' as company, 'Sin Asignar' as monetizable, 'Sin Asignar' as termino_pago, -1 as dias_disputa, -1 as limite_credito, -1 as limite_compra
...
ORDER BY vendedor ASC, operador ASC";
        $cuerpo="<table >
                    <thead>
                        <tr>
        ...
        if($vendedores!=null)
        {
            ...
            foreach ($vendedores as $key => $vendedor)
            {
                ...
                $cuerpo.="<tr>
                            ...
                            <td style='".self::colorVendedor($vendedor->vendedor,$vendedor->company)."'>".$vendedor->company."</td>
                            <td style='text-align: right;".self::colorVendedor($vendedor->vendedor,$vendedor->termino_pago)."'>(".$vendedor->termino_pago.")</td>
                            <td style='".self::colorVendedor($vendedor->vendedor,$vendedor->monetizable)."'>".$vendedor->monetizable."</td>
                            <td style='".self::colorVendedor($vendedor->vendedor,NULL,$vendedor->dias_disputa)."'>".$vendedor->dias_disputa."</td>
                            <td style='".self::colorVendedor($vendedor->vendedor,NULL,$vendedor->limite_credito)."'>".$vendedor->limite_credito."</td>
                            <td style='".self::colorVendedor($vendedor->vendedor,NULL,$vendedor->limite_compra)."'>".$vendedor->limite_compra."</td>
                        </tr>";
            } 
        }
        else
