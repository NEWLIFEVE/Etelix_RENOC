...
SELECT SUM(x.total_calls), SUM(x.complete_calls), SUM(x.minutes), (SUM(x.complete_calls)*100/SUM(x.total_calls)) AS asr, (SUM(x.minutes)/SUM(x.total_calls)) AS acd, SUM(x.pdd), SUM(x.cost), SUM(x.revenue), SUM(x.margin), (((SUM(x.revenue)*100)/SUM(x.cost))-100) AS margin_percentage
