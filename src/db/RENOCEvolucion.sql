...
FROM balance
WHERE date_balance>='$fecha'::date - '15 days'::interval AND id_carrier_supplier<>(SELECT id FROM carrier WHERE name='Unknown_Carrier') AND id_destination<>(SELECT id FROM destination WHERE name='Unknown_Destination') AND id_destination IS NOT NULL
...
SELECT date_balance, SUM(minutes) AS minutes, CASE WHEN SUM(revenue-cost)<SUM(margin) THEN SUM(revenue-cost) ELSE SUM(margin) END AS margin
FROM balance
WHERE date_balance>='$fecha'::date - '60 days'::interval AND id_carrier_supplier<>(SELECT id FROM carrier WHERE name='Unknown_Carrier') AND id_destination<>(SELECT id FROM destination WHERE name='Unknown_Destination') AND id_destination IS NOT NULL
GROUP BY date_balance
ORDER BY date_balance DESC
...
FROM(SELECT date_balance, SUM(minutes) AS minutes, CAST(0 AS double precision) AS margin
     ...
     FROM balance
     WHERE date_balance>='$fecha'::date - '15 days'::interval AND id_carrier_supplier<>(SELECT id FROM carrier WHERE name='Unknown_Carrier') AND id_destination<>(SELECT id FROM destination WHERE name='Unknown_Destination') AND id_destination IS NOT NULL AND margin<0
...
FROM(SELECT date_balance, SUM(minutes) AS minutes, CAST(0 AS double precision) AS margin
     ...
     SELECT date_balance, CAST(0 AS double precision) AS minutes, CASE WHEN SUM(revenue-cost)<SUM(margin) THEN SUM(revenue-cost) ELSE SUM(margin) END AS margin
     FROM balance
     WHERE date_balance>='$fecha'::date - '60 days'::interval AND id_carrier_supplier<>(SELECT id FROM carrier WHERE name='Unknown_Carrier') AND id_destination<>(SELECT id FROM destination WHERE name='Unknown_Destination') AND id_destination IS NOT NULL AND margin<0
     GROUP BY date_balance)b
GROUP BY b.date_balance
ORDER BY b.date_balance DESC
