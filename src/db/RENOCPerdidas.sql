...
FROM(SELECT id_carrier_customer, id_destination_int, id_carrier_supplier, CASE WHEN SUM(revenue-cost)<SUM(margin) THEN SUM(revenue-cost) ELSE SUM(margin) END AS margin, SUM(minutes) AS minutes, SUM(cost) AS cost, SUM(revenue) AS revenue
  FROM balance
